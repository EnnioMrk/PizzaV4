...
(async () => {
    ...
    //get session id from window url query
    ...
    pb.collection('sessions').subscribe(
        sessionId,
        async (e) => {
            //if (priority != e.record.priority) changePriority(e.record.priority);
            ...
            fetch('/api/orders/get', {
                ...
                body: JSON.stringify({
                    id: e.record.orders[e.record.orders.length - 1],
                }),
            })
                .then((res) => res.json())
                .then((order) => {
    ...
    function loadOrder(order) {
        ...
        if (order.options.length == 0) {
            ...
            order.options.forEach((optionName) => {
                let optionElement =
                    optionTemplate.content.cloneNode(true).children[0];
                optionElement.innerHTML = optionName;
                orderOptions.append(optionElement);
            });
        }
        let orderIngredients = orderElement.querySelector(
            '.ingredientsWrapper'
        );
        ...
        while (orders.length - 1 != queuePowerZero.length) {
            for (let i = 1; i < orders.length; i++) {
                if (orders[i].queuePower >= orders[i - 1].queuePowerToPass) {
                    ...
                    orders[i] = b;
                    console.log(
                        `${orders[i - 1].name} used ${
                            orders[i].queuePowerToPass
                        } of his ${
                            orders[i - 1].queuePower +
                            orders[i].queuePowerToPass
                        } queuePower to pass ${
                            orders[i].name
